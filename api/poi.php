...
foreach ($rs->haltes as $halte) {
	echo "{$halte->coordinaten->lng},\t{$halte->coordinaten->lat},\t{$halte->haltenummer}\n";
}
...
foreach ($rs->parkeerplaatsen as $parkeerplaats) {
	...
	echo "{$parkeerplaats->coordinaten->lng},\t{$parkeerplaats->coordinaten->lat},\t{$parkeerplaats->naam}\n";
}

$json = file_get_contents("http://api.qcommerz.nl/attracties/");
$rs = json_decode($json);
foreach ($rs->attracties as $attractie) {
	echo "{$attractie->coordinaten->lng},\t{$attractie->coordinaten->lat},\t{$attractie->naam}\n";
}
